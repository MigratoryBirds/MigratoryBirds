...
df_info = df_general_data.drop(columns=['Year', 'Model', 'Laydate_first_egg',
                      'Site', 'Days_from_LD',
                      'Rebuild_original', 'lat', 'long', 'Unnamed: 0'])
df_info_and_clusters = pd.merge(df_info, df_clusters, left_on='NestID', right_on='NestID')

df = df_info_and_clusters.sort_values(by=['Year','Date_trial'])

fig00 = px.scatter(x=df['Date_nest_found'], y= df['SnapsRasps'], color=df['ClusterID'])
fig00.update_layout(title='SnapsRasps counted in each nest',
              xaxis_title='Date when nest found',
              yaxis_title='Number of SnapsRasps',
              font=dict(size=25))
fig00.update_xaxes(rangeslider_visible=True)

df = df.drop(columns=['Date_trial','Date_nest_found'])

fig0 = px.parallel_coordinates(df,  color='ClusterID', labels={'ClusterID': 'Cluster ID', 'Propensity': 'Propensity',
...
fig1 = px.parallel_coordinates(df,  color='ClusterID', labels={'ClusterID': 'Cluster ID', 'Propensity': 'Propensity',
                              'SnapsRasps': 'SnapsRasps', 'Cuckoo_perch': 'Cuckoo Perch', 'Year': 'Year' 
                              }, color_continuous_midpoint=3, color_continuous_scale=px.colors.diverging.Tealrose)

# Cluster maps with and without animation.
clusterId = []
year = []
cluster_size = []
lat_centroid = []
...
fig2 = px.scatter_mapbox(df3, center={'lat': lat_coord , 'lon': long_coord}, lat='Lat_centroid', lon='Long_centroid', color='ClusterID', size='Cluster_size',
...
fig3 = px.scatter_mapbox(df3, center={'lat': lat_coord , 'lon': long_coord}, lat='Lat_centroid', lon='Long_centroid', color='ClusterID', size='Cluster_size',
    color_continuous_scale=px.colors.cyclical.IceFire, size_max=20, zoom=10,
    animation_group='ClusterID',hover_data=['ClusterID', 'Year'], mapbox_style='carto-positron')

if args[1] == 'unix':
  map.save('resources/visualisations/clustermap.html')
  fig00.write_html('resources/visualisations/cluster_SnapsRasps_with_range.html', auto_open=False)
  fig0.write_html('resources/visualisations/clusterdata.html', auto_open=False)
  fig1.write_html('resources/visualisations/clusterdata2.html', auto_open=False)
  fig2.write_html('resources/visualisations/clustermap_with_animation.html', auto_open=False)
  fig3.write_html('resources/visualisations/clustermap_without_animation.html', auto_open=False)
elif args[1] == 'windows':
  map.save('resources\visualisations\clustermap.html')
  fig00.write_html('resources\\visualisations\\cluster_SnapsRasps_with_range.html', auto_open=False)
  fig0.write_html('resources\\visualisations\\clusterdata.html', auto_open=False)
